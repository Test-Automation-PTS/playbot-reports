...
'use strict';
let model = JSON.parse(document.getElementById('results').textContent);
const $ = id => document.getElementById(id);
const escapeHtml = value => String(value ?? '').replace(/[&<>"']/g, c => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
const outcome = test => test.comparison ? (test.comparison.equal ? 'equal' : 'different') : 'none';
let assetBase = new URL('./', location.href).href;
const asset = (test, name) => new URL(`cases/${encodeURIComponent(test.id)}/${encodeURIComponent(name)}`, assetBase).href;
...
const initialBase = assetBase;
let timeline = [...(model.history || [])];
let loadSequence = 0;

function applyTimeline() {
  for (const test of model.cases) {
    test.history = timeline.filter(run => run.tests?.[test.full_name]).map(run => ({
      ...run.tests[test.full_name], run_number: run.run_number, url: `${run.url}#${test.id}`
    }));
  }
}
function navigationError(message) {
  let element = $('navigation-error');
  if (!element) { element=document.createElement('p');element.id='navigation-error';element.className='callout error';element.setAttribute('role','alert');$('trend').after(element); }
  element.textContent=message;element.hidden=!message;
}
function archivedUrl(value) {
  const url = new URL(value, initialBase);
  if (!['http:', 'https:'].includes(url.protocol) || url.origin !== location.origin) throw new Error('Archive must use this report origin');
  return url;
}
async function loadRun(value, updateHistory=true) {
  const sequence=++loadSequence;
  try {
    let next=initialModel, base=initialBase, target;
    if (value) {
      target=archivedUrl(value);
      base=new URL('./',target).href;
      const response=await fetch(new URL('results.json',base), {cache:'no-cache'});
      if (!response.ok) throw new Error(`HTTP ${response.status}`);
      next=await response.json();
      if (!Array.isArray(next.cases) || !Array.isArray(next.history)) throw new Error('Invalid report data');
    }
    if (sequence!==loadSequence) return;
    model=next;assetBase=base;selected=target?.hash.slice(1) || selected;
    // Keep the latest timeline while rendering the selected run's own results and assets.
    if (model.history.length>timeline.length) timeline=[...model.history];
    applyTimeline();navigationError('');
    if (updateHistory) {
      const url=new URL(location.href);
      if (value) url.searchParams.set('run',base); else url.searchParams.delete('run');
      url.hash=selected || '';history.pushState(null,'',url);
    }
    renderOverview();renderList();
  } catch(error) {
    if (sequence===loadSequence) navigationError(`The archived report could not be loaded (${error.message}). The displayed results are unchanged.`);
  }
}
// Intercept archive links, including per-test history, without discarding the viewer.
document.addEventListener('click',event=>{
  const link=event.target.closest('a[data-run-url]');
  if (!link || event.ctrlKey || event.metaKey || event.shiftKey || event.altKey) return;
  event.preventDefault();loadRun(link.dataset.runUrl);
});
window.addEventListener('popstate',()=>loadRun(new URL(location.href).searchParams.get('run'),false));
...
function renderDetail(test) {
  ...
  $('detail').innerHTML=`<div class="detail-top">${badge(test.status,test.status.toLowerCase())}${badge(test.category,'')}${badge(outcome(test)==='none'?'No comparison':`Comparison: ${outcome(test)}`,outcome(test))}<small>${test.duration_seconds==null?'':`${test.duration_seconds}s`}</small></div>
    ...
    <details><summary>Test history (${(test.history || []).length} runs)</summary><div class="test-history">${(test.history || []).map(run=>`<a data-run-url="${escapeHtml(run.url)}" href="${escapeHtml(run.url)}">${badge(run.status,run.status.toLowerCase())} Run #${run.run_number} · ${run.duration_seconds}s</a>`).join('')}</div></details>
...
if (requestedRun) loadRun(requestedRun,false);
if (model.timeline_url && location.protocol !== 'file:') {
  fetch(model.timeline_url,{cache:'no-cache'}).then(response=>response.ok?response.json():[]).then(latest=>{
    if (Array.isArray(latest) && latest.length>=timeline.length) {
      timeline=latest;applyTimeline();renderOverview();renderList();
    }
  }).catch(()=>{}); // Downloaded/offline reports keep their embedded history.
}
